...
class MillionaireGame:
    ...
    def setup_ui(self):
        ...
        # Left frame for question and answers
        left_frame = tk.Frame(main_frame, bg='#282c34')
        left_frame.pack(side='left', padx=10)

        # Right frame for lifelines and progress
        right_frame = tk.Frame(main_frame, bg='#282c34')
        right_frame.pack(side='right', padx=20)

        # Question box (set a fixed height)
        self.question_label = tk.Label(left_frame, text='', wraplength=400, justify='center',
                                       font=('Copperplate Gothic', 16), bg='#282c34', fg='white',
                                       height=4)  # Fixed height
        self.question_label.pack(pady=20)

        # Answer buttons in 2x2 grid with fixed size
        self.answer_buttons = []
        answer_frame = tk.Frame(left_frame, bg='#282c34')
        answer_frame.pack(pady=20)
        for i in range(2):
            for j in range(2):
                btn = tk.Button(answer_frame, text='', width=25, height=2,
        ...
        self.timer_bar.pack(fill='x', pady=5)  # Fixed height

        # Lifeline buttons with images and horizontal layout
        ...
        # Progress indicator with border around it
        ...
        for i in range(15):
            level_lbl = tk.Label(self.progress_frame, text=f'{15 - i}', bg='#3b3f45', fg='white',
                                 font=('Copperplate Gothic', 12), anchor='w', width=10)  # Adjust width
            ammount_lbl = tk.Label(self.progress_frame, text=f'{money_values[i]}', bg='#3b3f45', fg='white',
                                   font=('Copperplate Gothic', 12), anchor='e', width=10)  # Adjust width

            # Place them side by side using grid
    ...
    def check_answer(self, index):
        ...
        if self.answers[index] == self.current_question['CorrectAnswer']:
            ...
            messagebox.showerror('Incorrect!', 'Game Over.')
            self.root.quit()
    ...
    def use_swap(self):
        ...
        # Choose a new question randomly from remaining questions
        self.current_question = random.choice(level_questions)
    ...
    def use_jump_the_question(self):
        ...
        self.next_question()  # Load the next question
